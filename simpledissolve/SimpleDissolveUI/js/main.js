(function () {
	...
	var gWidthRatio = 1;
	var gFittedPreviewRect = {x:0, y:0, width:0, height:0};
    var gFittedSelectionRect = {x:0, y:0, width:0, height:0};
...
//------------------------------------------------------------------------------
	...
	function loadPreview() {
		...
		var previewSize = rescale({width: cnvsWidth, height: cnvsHeight},{width: gPreviewInfo.width, height: gPreviewInfo.height});
...
//------------------------------------------------------------------------------
	...
	function requestApplyDissolve(event) {
		...
		if (gPreviewInfo.percent > 0) {
			csInterface.requestOpenExtension("SimpleDissolveApplyFilter","")
			setTimeout(function(){dispatchEvent("com.adobe.event.applyDissolve", JSON.stringify(gPreviewInfo))},500)
		}
		else {
			csInterface.closeExtension();
		}
